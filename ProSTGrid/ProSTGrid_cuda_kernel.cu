...
namespace{
  ...
  __global__ void make_base_grid_5D_kernel(
    torch::PackedTensorAccessor<scalar_t,5,torch::RestrictPtrTraits,size_t> base_grid,
    ...
    float src_det_len
      ...
      float tmp_range_z;
      float idx, idy;
      if (r < base_grid.size(1) && c < base_grid.size(2)){
        ...
        for(size_t bn=0; bn<N; ++bn){
          for (size_t rc_ind = 0; rc_ind < inp_rc_pts; ++rc_ind){
            ...
            base_grid[bn][r][c][rc_ind][0] = tmp_range_x;
            base_grid[bn][r][c][rc_ind][1] = tmp_range_y;
            base_grid[bn][r][c][rc_ind][2] = tmp_range_z;
            base_grid[bn][r][c][rc_ind][3] = 1;
          }
        }
      }
    }
...
torch::Tensor base_grid_generator_5D_cuda_forward(
  ...
  bool align_corners
    ...
    //Calculate maximum ray casting sampling points
    ...
    auto base_grid = torch::empty({N, H, W, rc_pts, 4}, theta.options());
    ...
    AT_DISPATCH_FLOATING_TYPES(base_grid.type(), "base_grid_generator_5D_cuda_forward", ([&] {
    make_base_grid_5D_kernel<scalar_t><<<dimGrid, dimBlock>>>(
        base_grid.packed_accessor<scalar_t, 5, torch::RestrictPtrTraits,size_t>(),
